            ...
            <form action="../controller/purchase_controller.php?status=edit_grn" method="post">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <input type="hidden" name="grn_id" value="<?php echo $purchaserow["grn_id"] ?>">
                        <select name="po_id" class="form-control" required>
                            <option value="">Select PO</option>
                            <?php while ($po = $poList->fetch_assoc()) { ?>
                                <option value="<?php echo $po['po_id']; ?>"
                                        <?php if ($po['po_id'] == $purchaserow['po_id']) echo 'selected'; ?>>
                                    PO #<?php echo $po['po_id']; ?> - <?php echo $po['po_date']; ?>
                                </option>
                            <?php } ?>
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="grn_date" class="form-control" value="<?php echo $purchaserow["grn_date"]; ?>" required>
                    </div>
                </div>
...
<script>
    ...
    const xhr = new XMLHttpRequest();
    xhr.open("GET", "../controller/purchase_controller.php?status=get_grn_items_for_edit&grn_id=" + grnId, true);
    xhr.onload = function () {
        document.getElementById("poItemsContainer").innerHTML = this.responseText;
    };
    xhr.send();
